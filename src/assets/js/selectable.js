...
export default class selectable {
    ...
    mouseDown(e) {
        ...
        if(newPath.indexOf('ctx-menu-container') > 0 || newPath.indexOf('ivu-modal-wrap') > 0) { 
            return ;
        }
        // this.selectables = [];
        // this.selectables = Array.prototype.slice.call(document.querySelectorAll('.selectable'))
        if (e.button !== 0) {
            return;
        }
        ...
        if (!this.addMode) {
            this.selecting = this.selectables = []; // reset all selection
            this.selected = this.selecting;
            if (typeof this.selectedSetter === 'function') {
                this.selectedSetter(this.selected, this.selecting);
            }
        } else if (typeof this.selectedGetter === 'function') {
            let arr = this.selectedGetter() || [];
            let selectables = Array.prototype.slice.call(document.querySelectorAll('.selectable'))

            let gotSelection = [];
            for (let i = 0; i < arr.length; i++) {
                if (arr[i] != false) {
                    gotSelection.push(arr[i])
                }
            }
            /*console.log(gotSelection);
            for (let i = 0; i < selectables.length; i++) {
                let id = selectables[i].dataset.id;
                if (gotSelection.indexOf(id) == -1) {
                    gotSelection.push(id);
                }
            }*/
            // console.log(this.selected);
            this.selected = arr
                // this.selected = this.selectables.map((v, i) => !!gotSelection[i]);
                // this.selected = gotSelection;
                // this.selected = this.selecting;
                // if (typeof this.selectedSetter === 'function') {
                // this.selectedSetter(this.selected, this.selecting);
                // }
                // this.selectedSetter(gotSelection);
        }
        ...
        if (this.dragging) {
            ...
            if (this.addMode) {
                ...
                for (let i = 0; i < this.selected.length; i++) {
                    if (this.selected[i] != true && this.selected[i] != false && typeof(this.selected[i]) != undefined && arr.indexOf(this.selected[i]) == -1) {
                        arr.push(this.selected[i])
                    }
                }
                for (let i = 0; i < this.selecting.length; i++) {
                    if (this.selecting[i] != true && this.selecting[i] != false && typeof(this.selecting[i]) != undefined && arr.indexOf(this.selecting[i]) == -1) {
                        arr.push(this.selecting[i])
                    }
                }
                this.selected = this.selecting = arr;
                // }
            } else {
                this.selected = this.selecting;
            }
    ...
    updateSelection() {
        ...
        let selecting = this.selectables.map(selectable.absBox).map(b =>
            ((Math.abs((s.left - b.left) * 2 + s.width - b.width) < (s.width + b.width)) &&
                (Math.abs((s.top - b.top) * 2 + s.height - b.height) < (s.height + b.height))) ? b.id : false
        );
        let arr = [];
        ...
        for (let i = 0; i < selecting.length; i++) {
            if (selecting[i] != false) {

                if (arr.indexOf(selecting[i]) > -1) {
