...
utils.extend(Uploader.prototype, {
  ...
  uploadNextChunk: function (preventEvents) {
    ...
    // console.log('checkChunkUploaded:'+checkChunkUploaded);
    if (this.opts.prioritizeFirstAndLastChunk) {
      utils.each(this.files, function (file) {
        if (file.paused || file.isGetMd5) {
          return
        }
    ...
    utils.each(this.files, function (file) {
      if (!file.paused) {
        ...
        utils.each(file.chunks, function (chunk) {
          let status = chunk.tested == true ? false : true;
          if (chunk.status(status) === pendingStatus) {
            chunk.send()
            found = true
            return false
          }
